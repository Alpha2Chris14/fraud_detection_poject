...
def verify_account_details(request):
    return render(request,'homeApp/verify_account_details.html')

# this makes the api call
def submit_verify_account_details(request):        
    url = "http://127.0.0.1:8080/api/v1/fraud/store"
    ...
    files = {
        "proof": (request.FILES['proof'].name, request.FILES['proof']),
    }

    headers = {
        "Authorization": "Bearer sk_test_b753e53c845f53c2923fdf3d818b9e0c5ca36fbb"
    }

    response = requests.post(url, data=payload, files=files, headers=headers)

    if response.status_code == 200:
        return JsonResponse({"message": "API call succeeded"})  # Return a JSON response
    else:
        return JsonResponse({"error": "API call failed"}, status=response.status_code) 

def confirm_account_fraud(request):
    return render(request,'homeApp/confirm_account_fraud.html')

def bank(request):
    return render(request,'homeApp/bank.html')

def approve(request):
    return render(request,'homeApp/approve.html')
